...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        let currentV :NSString = dic[key] as! NSString
        
        var runV : NSString
        if  (NSUserDefaults.standardUserDefaults().stringForKey(key) != nil)  {
            
            runV = NSUserDefaults.standardUserDefaults().stringForKey(key)!
        }else {
            runV = ""
        }
        
        //判断版本
        if currentV != runV || runV.length <= 0 {
            
            NSUserDefaults.standardUserDefaults().setValue(currentV, forKey: key)
            
            self.window?.rootViewController = WelcomViewController()
